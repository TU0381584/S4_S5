...
Table~\ref{tab:related} summarises this gap against the seven works
closest to our own three pillars, each named for the specific claim it
makes and the specific gap it leaves against the three practice checks
this paper applies throughout; a further, closely related
set~\cite{mohajer2026pronto,bouroudi2025gnnmarl,qasim2025tgnnfl,gong2026femaddpg}
combines two of these three pillars but, like every work in the table,
evaluates exclusively on its own reported outcome metric, computed
once, on one seed or run.

\begin{table}[t]
\caption{Closest prior art against this paper's three practice checks. \cmark/\xmark: whether the work checks its own metric for degenerate-policy masking, replicates a headline finding on an independent seed sample, and validates on a live deployment.}
\label{tab:related}
\centering
\small
\begin{tabular}{@{}p{1.9cm}p{2.5cm}p{3.1cm}ccc@{}}
\toprule
Work & Setting & Focus & \makecell{Metric-\\mask.} & \makecell{Indep.\\seed} & \makecell{Live\\deploy.} \\
\midrule
Shao et al.~\cite{shao2021gatmarl} & Dense-cellular slicing, DQN/A2C baselines & GAT strengthens cooperation (our encoder's precedent) & \xmark & \xmark & \xmark \\
Ahmadi et al.~\cite{ahmadi2024generalizable} & Multi-agent slicing, unseen topologies & Topology generalisation via GAT & \xmark & \xmark & \xmark \\
Sulaiman et al.~\cite{sulaiman2023coordinated} & Joint slicing + admission, substrate network & Closest prior art to our own problem & \xmark & \xmark & \xmark \\
Tashman \& Cherkaoui~\cite{tashman2026adversarial} & Adversarial robustness & Robustness to a disruptive condition & \xmark & \xmark & \xmark \\
Fatehi et al.~\cite{fatehi2026attentionmarl} & Attention MARL, latency spikes & Robustness to a disruptive condition & \xmark & \xmark & \xmark \\
Zhang et al.~\cite{zhang2022federatedoran} & Federated O-RAN xApp coordination & Closest periodic-sync precedent; no DP or quality check & \xmark & \xmark & \xmark \\
Yasin et al.~\cite{yasin2025dpfl} & FL+DP for O-RAN RIC security & Different threat model (securing controller) & \xmark & \xmark & \xmark \\
\textbf{This paper} & Multi-gNB slice admission control & GAT-CTDE + FL/DP + disruption, per-component & \cmark & \cmark & \cmark \\
\botrule
\end{tabular}
\end{table}

Ahmadi et al.'s and Sulaiman et al.'s own topology-generalisation questions are complementary to, not answered by, the fixed-topology evaluation-integrity question we ask; Section~\ref{sec:results-m6} tests the complementary direction directly, asking whether correctness-aware metrics remain reliable as cluster size and adjacency sparsity change rather than whether a policy generalises to an unseen topology. Tashman and Cherkaoui's and Fatehi et al.'s robustness studies motivate our own disruption campaign (Section~\ref{sec:results-m4}), but neither checks whether the metric reporting robustness is itself trustworthy or replicates a robustness finding on an independent sample, exactly the gap our own campaign's independent-seed replication closes -- not hypothetically, since it is what overturned our initial churn-immunity reading. Zhang et al.'s federated coordination precedent has no differential-privacy step or correctness-aware metric to check whether federating changed decision quality, which our own federated campaign (Section~\ref{sec:results-m3}) reports directly; Yasin et al.'s closest FL+DP precedent targets a different threat model (securing the controller) rather than characterising a control policy's own privacy-utility trade-off. No work in Table~\ref{tab:related} clears all three of the practice gaps the Introduction names as this paper's own to close.
...
summarises each against the safeguard that caught it, three diagnosed
in this section and the fourth in the live deployment
(Section~\ref{sec:results-m8}). We rely throughout on two
correctness-aware metrics computed from data every run already logs:
mean reward per step, the actual training objective, and block
...
\centering
\small
\begin{tabular}{@{}p{2.3cm}p{3.0cm}p{2.7cm}p{2.7cm}@{}}
\toprule
Failure & How a standard pipeline misses it & Safeguard that caught it & Outcome \\
\midrule
Training collapse (this section) & Compliance metric scores an always-accept no-op no worse than correct behaviour & Correctness-aware metric pair & 0/30 $\to$ 21/30 seeds differentiate (Fig.~\ref{fig:collapse}) \\
Eval-log contamination (this section) & Append-mode logging silently stacks retraining runs & Per-log episode-index audit & Logs cleared, one re-eval, no retrain needed \\
Same-seed reproducibility gap (Section~\ref{sec:results-m4}) & A single seed sample looks like a confirmed architectural property & Reproduction-vs-replication discipline & Independent-DQN churn-immunity claim retracted \\
Live scheduler-floor misconfiguration (Section~\ref{sec:results-m8}) & Config-level review cannot see a cross-layer scheduler floor & Direct scheduler-state instrumentation & $>$100,000 readings confirm the floor clear post-fix \\
\botrule
\end{tabular}
\end{table}

While generating a privacy-utility sweep for the federated variant (Section~\ref{sec:results-m3}), noise levels that should have produced genuinely different, stochastically-trained checkpoints instead produced bit-identical per-seed evaluation compliance, tracing back to our first GAT-CTDE encoder's eval policy blocking zero requests across every evaluation episode on all 30 campaign seeds -- a genuine failure, not a legitimate optimum, confirmed by a direct Q-value probe showing $Q(\text{accept})-Q(\text{reject})$ positive and large for every slice including mMTC, while the independent-DQN ablation (Section~\ref{sec:results-m2}), which sees identical raw features through no encoder at all, does not exhibit this collapse; this isolates the encoder, not the reward or training loop, as the fault.
...
Two targeted fixes, a per-layer \texttt{LayerNorm} constraining every node's embedding to zero-mean, unit-variance regardless of input magnitude, followed by the per-slice Q-heads of Section~\ref{sec:methodology-perslice}, progressively reduce the collapse from 30/30 to 27/30 to a final 9/30 seeds (Fig.~\ref{fig:collapse}); not every plausible variant helped along the way (one, reverted, made results worse), and we report the one that did not rather than omit it.
...
A separate, unrelated bug was caught while building an evaluation-time disruption harness: our append-mode logging utility never truncated its output file, and GAT-CTDE's three in-place retrainings during the collapse diagnosis above each wrote to the same per-seed path, so every one of the 30 seeds' evaluation logs had silently stacked all three runs' episodes together (confirmed by each log's own episode index resetting to 1 twice) -- a hidden three-way average across two obsolete runs and the true final one, affecting every GAT-CTDE number through an earlier draft and every M3 result reusing it as a reference point. The saved checkpoints themselves were unaffected (a different, overwriting save routine, verified by reproducing one seed's final-block statistics in isolation), so the fix needed no retraining: every stale log was cleared, evaluation re-run once against the already-trained checkpoints, and the training script hardened to clear its output path before any future run. All numbers in this paper reflect that corrected data.
...
Table~\ref{tab:disruption} reports each arm's own per-severity cost
where an individual triple was computed; every dropout and churn cost
accelerates with severity rather than growing proportionally (Wilcoxon
$p\leq0.03$, 9 or 10 of 10 seeds hurt in every case), while demand
spike shows no threshold at all on block precision, flat and
near-ceiling through the highest tested multiplier for every arm.
...
\begin{tabular}{@{}lccc@{}}
\toprule
Arm & Dropout (10/30/60\%) & Churn (10/30/60\%) & Spike thresholds? \\
\midrule
GAT-CTDE & +0.165/+0.601/+1.710 & +0.059/+0.200/+0.529 & \xmark \\
Federated ($\sigma$=0) & accelerates$^\dagger$ & +0.078/+0.246/+0.585 & \xmark \\
Independent DQN & accelerates$^\dagger$ & +0.000/+0.168/+0.640 & \xmark \\
Single-agent DQN & accelerates$^\dagger$ & $\ddagger$ & \xmark \\
\botrule
\end{tabular}
\end{table}

The federated arm's own churn cost above was regenerated post-hoc from the still-intact frozen checkpoint after the original per-seed logs were found missing during this paper's own reproducibility audit, with environment drift over the intervening months identified as the cause by systematic elimination of every code- and data-level alternative. Independent DQN's own churn cost needs both samples reported together to state honestly: the committed value above (900--909) is small and not conventionally significant ($p=0.084/0.106/0.065$), which an earlier pass read as independent DQN being architecturally immune to churn, since it never attends to another agent's features. An independent-seed replication (seeds 1000--1009, disjoint from every other result in this paper) contradicts that reading directly: independent DQN's churn cost there is significant at every severity (+0.097/+0.657/+1.223, $p=0.0020$ throughout) and \emph{larger} than GAT-CTDE's or the federated arm's at matched severities, the opposite of immune (Table~\ref{tab:replication} below reports both samples together). We do not conclude independent DQN is immune to churn, since the direction was consistent across both samples (churn always hurts, never helps). What was wrong was treating a borderline, underpowered $n=10$ result as a confirmed architectural property, and this section reports the corrected reading, not the retracted one, as this paper's discipline of checking every headline result against an independent seed sample, not the same seeds reproduced again, requires.

A reward-based signal that spike \emph{improves} per-step reward is real and reproduces across both seed samples for GAT-CTDE, independent DQN, and the federated arm ($p\leq0.014$ in both), but traces to a fixed per-step violation penalty diluted by a larger request-volume denominator, not better decision-making. Block precision, a volume-invariant ratio, is the metric that actually answers whether spike corrupts decision quality, and it says no.
...
Every dropout and churn result above was independently checked against the 1000--1009 sample and holds up in direction and approximate magnitude; GAT-CTDE's own dropout costs replicate closely (+0.170/+0.552/+1.449, against the committed sample's +0.165/+0.601/+1.710). Table~\ref{tab:replication} reports this paper's four cross-section headline comparisons side by side against their own independent-seed replication. Three of the four hold up with overlapping confidence intervals across samples; the fourth, independent DQN's churn cost (flagged $\dagger$), does not, and is the retraction above.
...
across two small samples, was resolved rather than
