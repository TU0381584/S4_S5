...
Two targeted fixes -- a per-layer \texttt{LayerNorm} (zero-mean, unit-variance embeddings) and the per-slice Q-heads of Section~\ref{sec:methodology-perslice} -- progressively reduce the collapse from 30/30 to 27/30 to a final 9/30 seeds (Fig.~\ref{fig:collapse}); not every variant helped (one, reverted, made results worse), reported here rather than omitted.
...
recalibration that appeared to reverse a complete live collapse at
double load; re-collection under the fixed config shows that collapse
never reproduced once the floor bug was fixed at its root, so the
recalibration is retired (its anchors preserved, unmodified, as
evidence of the misdiagnosis, not deleted). A companion
